...
async function fetchBusinessInfo() {
  const { data, error } = await supabaseClient.from('business_info').select('*').single();
...
async function fetchFeatures() {
  const { data, error } = await supabaseClient.from('features').select('*').order('sort_order');
  if (error) { console.error('fetchFeatures:', error); return []; }
  return data;
}

async function fetchOrderingSteps() {
  const { data, error } = await supabaseClient.from('ordering_steps').select('*').order('step_number');
...
async function fetchAboutContent() {
  const { data, error } = await supabaseClient.from('about_content').select('*').order('paragraph_number');
...
async function updateBusinessInfo(info) {
  const { error } = await supabaseClient.from('business_info').update(info).eq('id', info.id);
  if (error) throw error;
}

async function updateCake(id, data) {
  const { error } = await supabaseClient.from('cakes').update(data).eq('id', id);
  if (error) throw error;
}

async function updateCakeImages(cakeId, images) {
  // Delete existing then re-insert
  await supabaseClient.from('cake_images').delete().eq('cake_id', cakeId);
  if (images.length > 0) {
    const { error } = await supabaseClient.from('cake_images').insert(
...
async function updateCakeDetails(cakeId, details) {
  await supabaseClient.from('cake_details').delete().eq('cake_id', cakeId);
  if (details.length > 0) {
    const { error } = await supabaseClient.from('cake_details').insert(
...
async function addFeature(feature) {
  const { error } = await supabaseClient.from('features').insert(feature);
  if (error) throw error;
}

async function updateFeature(id, data) {
  const { error } = await supabaseClient.from('features').update(data).eq('id', id);
  if (error) throw error;
}
...
async function addOrderingStep(step) {
  const { error } = await supabaseClient.from('ordering_steps').insert(step);
  if (error) throw error;
}

async function updateOrderingStep(id, data) {
  const { error } = await supabaseClient.from('ordering_steps').update(data).eq('id', id);
  if (error) throw error;
}

async function deleteOrderingStep(id) {
  const { error } = await supabaseClient.from('ordering_steps').delete().eq('id', id);
  if (error) throw error;
}

async function addTestimonial(t) {
  const { error } = await supabaseClient.from('testimonials').insert(t);
  if (error) throw error;
}

async function updateTestimonial(id, data) {
  const { error } = await supabaseClient.from('testimonials').update(data).eq('id', id);
  if (error) throw error;
}
...
async function updateAboutParagraph(id, content) {
  const { error } = await supabaseClient.from('about_content').update({ content }).eq('id', id);
  if (error) throw error;
}
...
async function uploadCakeImage(file, path) {
  const { data, error } = await supabaseClient.storage.from('cake-images').upload(path, file, { upsert: true });
  if (error) throw error;
  const { data: { publicUrl } } = supabaseClient.storage.from('cake-images').getPublicUrl(path);
  return publicUrl;
}
